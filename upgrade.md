...
- [ ] Создать `client/src/lib/app-capabilities.ts`
- [ ] Определять ОС (iOS, Android, Desktop)
- [ ] Определять браузер (Chrome, Safari, Firefox, etc.)
- [ ] Проверять поддержку функций приложения:
  - Доступ к камере
  - Геолокация (GPS)
...
- [ ] Создать `client/src/components/app-capabilities-dialog.tsx`
- [ ] Отображать информацию о браузере и ОС
- [ ] Список поддерживаемых функций с иконками ✓/✗
- [ ] Чекбокс "Не показывать это окно" (checked по умолчанию)
- [ ] Кнопка "Понятно" / "Continue"

### 4. Интеграция в страницу камеры
- [ ] Добавить состояние для показа диалога
- [ ] Проверять localStorage при первом запуске
- [ ] Показывать диалог только при первом использовании

### 5. Переводы
- [ ] Добавить переводы в en.ts
- [ ] Добавить переводы в ru.ts
...
|--------|--------|------|
| Обновление upgrade.md | ✅ Готово | 10.12.2025 |
| Утилита возможностей | ⏳ В работе | 10.12.2025 |
| AppCapabilitiesDialog | ⏳ Ожидает | - |
| Интеграция в камеру | ⏳ Ожидает | - |
| Переводы | ⏳ Ожидает | - |
