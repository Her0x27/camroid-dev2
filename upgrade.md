...
## Описание
1. **Фото на весь экран:** изображение занимает 100% экрана без отступов/рамки
2. **Хлебные крошки поверх фото:** header с прозрачным градиентом поверх изображения
3. **Иконки поверх фото:** footer с прозрачным градиентом поверх изображения
4. **Как в стандартном просмотрщике:** immersive режим просмотра

## Чек-лист задач v73

- [x] Обновить upgrade.md — добавить секцию v73
- [x] Убрать paddingTop и paddingBottom у контейнера изображения
- [x] Оптимизировать прозрачность header/footer
...
| Прозрачность панелей | ✅ Готово | 16.12.2025 |
...
- **Убран paddingTop: '72px'** — фото теперь начинается от верхнего края экрана
- **Убран paddingBottom: '52px'** — фото теперь доходит до нижнего края экрана

#### Изображение:
- **Класс `max-h-full` заменён на `h-full`** — фото занимает 100% высоты экрана
- **object-contain** — сохранение пропорций изображения

#### Header (хлебные крошки):
- **Позиция поверх фото** — absolute top-0, z-50
- **Прозрачный градиент** — bg-gradient-to-b from-black/80 via-black/60 to-transparent
- **Содержимое:** кнопка назад, breadcrumbs, счётчик, дата, кнопка камеры

#### Footer (иконки действий):
- **Позиция поверх фото** — absolute bottom-0, z-50
- **Прозрачный градиент** — bg-gradient-to-t from-black/80 via-black/60 to-transparent
