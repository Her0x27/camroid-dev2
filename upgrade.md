...
- Расширен INITIAL_MODULES: Система → Камера → GPS → Настройки → Готово
- Добавлен MODULE_NAMES с новыми шагами

### splash-screen.tsx
- Добавлена симуляция загрузки шагов с интервалом
- Каждый шаг отображается с анимацией в прогресс-баре
- Шаги загружаются последовательно для визуального эффекта

### App.tsx
- Добавлено состояние showWelcome
- Splash завершается → Welcome отображается
- Welcome закрывается → Камера запускается
- Камера не стартует до закрытия Welcome
