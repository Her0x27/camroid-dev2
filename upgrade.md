...
- [ ] Добавить тактическое фото для превью в assets

### 2. Компонент SettingsPreview
- [ ] Создать полноэкранный overlay компонент
- [ ] Рендерить прицел с текущими настройками (размер, толщина, прозрачность, цвет)
- [ ] Рендерить водяной знак с учётом активных опций:
  - GPS координаты (если GPS включен)
  - Высота, точность, азимут (демо-данные)
  - Заметка (если showMetadata включен)
  - Временная метка
- [ ] Масштабирование водяного знака по настройке watermarkScale

### 3. Контекст PreviewContext
- [ ] Создать контекст для управления состоянием превью
- [ ] Поддержка активации/деактивации превью
- [ ] Отслеживание какая секция активировала превью

### 4. Интеграция с секциями настроек
- [ ] Обновить ReticleSection — активация превью при взаимодействии со слайдерами
- [ ] Обновить WatermarkSection — активация превью при взаимодействии со слайдерами

### 5. Стилизация
- [ ] Glass effect для панели настроек когда превью активно
- [ ] Плавные анимации появления/исчезновения превью

### 6. Переводы
- [ ] Добавить переводы для демо-заметки превью (ru/en)
...
| Задача | Статус |
|--------|--------|
| Фото для превью | ⏳ В процессе |
| SettingsPreview компонент | ⏳ Ожидает |
| PreviewContext | ⏳ Ожидает |
| Интеграция с секциями | ⏳ Ожидает |
| Стилизация | ⏳ Ожидает |
| Переводы | ⏳ Ожидает |
