...
**Проблема:** Несмотря на извлечение хуков, файл остаётся большим. Много JSX разметки и inline логики.

**Предложение:** Извлечь дополнительные UI компоненты (GalleryToolbar, GalleryContent).

```
⬜ [ARCH-5] [ОПЦИОНАЛЬНО] Извлечь GalleryToolbar компонент для панели действий
⬜ [ARCH-6] [ОПЦИОНАЛЬНО] Извлечь GalleryContent компонент для основного содержимого
```
...
**Проблема:** Оба хука реализуют похожий паттерн:
- Отслеживание предыдущего значения через ref
- Порог значимого изменения (threshold)
- Throttling обновлений
- Cleanup подписок

**Предложение:** Создать общий хук `useSensorWithThreshold` для унификации логики.
...
| client/src/pages/camera/index.tsx | 638 | Требует дальнейшей декомпозиции |
